...
begin
  ...
  Try
    ...
    Fee := FAppParams.ParamByName[CT_PARAM_DefaultFee].GetAsInt64(CT_MinTransactionFee);
...
(*  if (ForceMining) Or
    ...
    if (cbAllowMining.checked) Or (ForceMining) then begin
      ...
      Stop;
      lblMinersClients.Caption := 'Not Mining';
    end;
//*  end else Stop;
end;
...
begin
  if TAccountComp.AccountPublicKeyImport(CT_MinerPublicKey, PublicK, err) then Result := PublicK
  else showMessage(err);

  { //* changed pbm   Result := CT_TECDSA_Public_Nul;
...
begin
  ...
  if isMining then begin
    ...
    lblBlocksFound.Caption := IntToStr(MinersBlocksFound);
    lblMinersClients.Caption := '1 Local Miner';
  end else begin
