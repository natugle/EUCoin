...
begin
  ...
  ebNewPublicKey.Text := '';
  ebFee.Text := TAccountComp.FormatMoney(CT_MinTransactionFee);
  FFee := CT_MinTransactionFee;
...
begin
  ...
  if (FFee < CT_MinTransactionFee) then begin
    FFee := CT_MinTransactionFee;
    ebFee.text := TAccountComp.FormatMoney(CT_MinTransactionFee);
  end;
  If not rbTransaction.Checked then begin
...
var wd : Boolean;
begin
  if Value < CT_MinTransactionFee then exit;
