...
begin
  ...
  FData.target := target;
  FData.amount := amount;
  if fee < CT_MinTransactionFee then fee := CT_MinTransactionFee; //* pbm need a minimal fee for mining etc
  FData.fee := fee;
...
begin
  ...
  FData.n_operation := n_operation;
  if fee < CT_MinTransactionFee then fee := CT_MinTransactionFee; //* pbm minimal fee for mining etc
  FData.fee := fee;
  FData.payload := payload;
  FData.public_key := key.PublicKey;
...
begin
  FData.account := account_number;
  FData.n_operation := n_operation;
  if fee < CT_MinTransactionFee then fee := CT_MinTransactionFee; //* pbm
  FData.fee := fee;
