    ...
    Try
      ...
      if (Operations.OperationBlock.block > 0) then begin
        if ((Operations.OperationBlock.timestamp) < (FLastOperationBlock.timestamp)) then begin
          ...
          exit;
        end; //* pbm changed only 15 sec future ??
        if (Operations.OperationBlock.timestamp > (UnivDateTimeToUnix(DateTime2UnivDateTime(now))+CT_MaxSecondsFutureBlockTime)) then begin
      ...
      //* pbm add check if key is CT_MinerPublicKey
      // Ok, include!
      // WINNER !!!
      // Congrats!
...
//* pbm max supply in genesis block
class function TPCBank.GetRewardForNewLine(line_index: Cardinal): UInt64;
Var n, i : Cardinal;
begin
  Result := CT_MinReward;
  if (line_index = 0) then Result := CT_FirstReward;
