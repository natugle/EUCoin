...
begin
  Result := FormatFloat('#,###0.00',(Money/100)); //* pbm only two after delimit
end;
...
begin
  ...
  try
    ...
    money := Round( StrToFloat(s)*100 ); //* pbm only 2 after delimiter
    Result := true;
  Except
    result := false;
  end;
...
begin
  ...
  for i := Low(Result.accounts) to High(Result.accounts) do begin
    ...
    Result.accounts[i].accountkey := accountkey; //* pbm add check if key is CT_MinerPublicKey
